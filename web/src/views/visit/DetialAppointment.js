...
import {
  ...
  Linking, Platform, DeviceEventEmitter, Image, TouchableOpacity, FlatList, Dimensions
} from 'react-native'
...
export default function DetialAppointment({ route }) {
  ...
  const callAPIGetDetail = () => {
    apiGetDetail(passingData?.id).then(async res => {
      if (res) {
        const parseData = await res.json()
        console.log('parseData: ', parseData)
        const getData = parseData?.survey || []
        setDetail(getData)
      }
    })
  }
