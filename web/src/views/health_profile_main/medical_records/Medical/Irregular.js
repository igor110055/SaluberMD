...
// import { saveDataIrre } from 'actions/common'
...
export default function Irregular() {
  ...
  const lsIrreRedux = StateLocal.medicalDataIrre || []
  ...
  const RenderItemLong = ({item, index, data}) => {
    const _onPressItemLong = () => {
      // NavigationService.navigate(Routes.NEW_DETAIL_IRREGULAR_SCREEN, {
      //   data: item,
      //   index: index
      // })
    }
        ...
        <View style={styles.shadow}>
          <FlatList
            ...
            renderItem={({item, index}) => (
              <RenderItemLong item={item} index={index} data={irregular} />
            )}
          />
        </View>
      )}
