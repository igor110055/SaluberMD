...
import {color040404, color0B40B1, color363636, color3777EE, colorDDDEE1, colorFFFFFF} from 'constants/colors'
...
export default function StepChart() {
  ...
  const callAPIList = () => {
    axios({
      method: 'get',
      url: `${APIs.hostAPI}backoffice/wellness/getWellnessStepsValuesTimezone/${valShow?.value}/-420/${valSource?.name || 'CORX'}`,
      headers: {
  ...
  const callAPIStepToday = () => {
    axios({
      method: 'get',
      url: `${APIs.hostAPI}backoffice/wellness/getTodayStepsByTimezone/${timeNow}/-420/${valSource?.name || 'CORX'}`,
      headers: {
        'content-type': 'application/json',
        'x-auth-token': token,
        'timezone': -420
