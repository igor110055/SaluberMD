...
export default function Provider({
  ...
  const callAPIDoctorBySpeciality = () => {
    ...
    axios({
      method: 'get',
      url: `${APIs.hostAPI}backoffice/findoctor/${specialityID}`,
      headers: {
        ...
        <ScrollView>
          ...
          <View style={styles.descriptionRadio}>
            <Text style={customTxt(Fonts.Regular, 16, color5C5D5E).txt}>
              {Translate(languageRedux).ITEM_SELECT}
            </Text>
          </View>
        </ScrollView>
      ...
      <View>
        ...
        <View style={styles.chooseSession}>
          <CustomTextInput
            ...
            placeholder={Translate(languageRedux).SELECT_DATE}
          />
        </View>
  ...
  const checkProvider = () => {
    if ((lsDoctor || []).length > 0 && requestSlot === false) {
