...
import {View, Text, StyleSheet, Image, ScrollView,
  TouchableOpacity, DeviceEventEmitter, Platform} from 'react-native'
...
export default function DetailReminder({ route }) {
  ...
  const idDetail = route?.params?.idDetail
  ...
  const [dataRemider, setDataReminder] = useState()
  ...
  const filterData = () => {
    var dataFilter = passingData.filter((val) => val?.idDetail === idDetail)
    console.log('dataFilter: ', dataFilter)
    if ((dataFilter || []).length > 0) {
      var dataObject = dataFilter[0]
    }
    setDataReminder(dataObject)
    setLoading(false)
  }
  ...
  const _onChangeTimeAndroid = (event, selectedDate) => {
    const currentDate = selectedDate || date
    setShowTime(false)
    setDate(currentDate)
    setSince(selectedDate?.nativeEvent?.timestamp)
  }
    ...
    <View style={styles.container}>
      ...
      {isConfirm && (
        <ConfirmView
          ...
          since={since}
          onPressSince={() => {
            Platform.OS === 'ios' ? datePickerRef.current.onPressDate() : setShowTime(true)
          }}
