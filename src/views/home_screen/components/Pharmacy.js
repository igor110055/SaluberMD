...
import { View, Text, StyleSheet, Image, TouchableOpacity, FlatList, Linking, Platform, Alert } from 'react-native'
...
export default function Pharmacy() {
  ...
  const RenderItem = ({ item, index }) => {
    const _onPressPhone = () => {
      let phoneNumber = ''
      console.log('phone: ', item?.biz_phone)
      if (Platform.OS === 'android') {
        phoneNumber = `tel:${+item?.biz_phone}`
      }
      else {
        phoneNumber = `tel:${+item?.biz_phone}`
      }
    ...
    return (
      <View>
        ...
        <View style={styles.ctnBoxPharma}>
          ...
          {/* PHONE */}
          {item?.biz_phone && <TouchableOpacity onPress={_onPressPhone} style={styles.ctnPhone}>
          ...
          <View style={styles.ctnButtonLayout}>
            <TouchableOpacity onPress={() => {
              ...
              <Text style={customTxt(Fonts.Bold, 14, color3777EE).txt}>
                {Translate(languageRedux).addpharmacy}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
