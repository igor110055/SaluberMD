...
export default function Apointment() {
  ...
  const [lsData, setLsAppointment] = useState(cloneDeep(lsAppointmentRedux))
  ...
  const renderBoxAppointment = (index) => {
    ...
    const checkDay = () => {
      if (moment(lsData[index]?.startsAt).format('YYYY-MM-DD') === today) {
        return <Text style={customTxt(Fonts.SemiBold, 18, colorBDBDBD).txt}>{Translate(languageRedux).TODAY}</Text>
      }
      if (moment(lsData[index]?.startsAt).format('YYYY-MM-DD') === tomorrow) {
