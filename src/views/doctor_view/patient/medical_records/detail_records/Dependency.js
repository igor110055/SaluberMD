...
export default function Dependency({route}) {
    ...
    const checkWeanned = () => {
        if (passingData?.weaned_off === 0) {
          return Translate(languageRedux).no
        }
        if (passingData?.weaned_off === 1) {
      ...
      return (
        <View>
          <RenderItem
            ...
            content={passingData?.dependency}
          />
          {passingData?.dependencyId === 1 && (
            <RenderItem
          ...
          <RenderItem
            ...
            content={passingData?.yearStarted}
          />
          <RenderItem
            category={Translate(languageRedux).dailyuse}
            content={passingData?.daily_use}
          />
          <RenderItem
    ...
    return (
        ...
        <Header
          textCenter={Translate(languageRedux).dependencies}
