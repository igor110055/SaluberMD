...
import {View, Text, StyleSheet, TouchableOpacity, Image, DeviceEventEmitter, FlatList} from 'react-native'
...
export default function AppointmentRequest({data, onPressRequestAppointment, setValueItemRequest}) {
      ...
      <TouchableOpacity onPress={onPress} style={styles.ctnBoxAppointment}>
        <View style={styles.line1Box}>
          ...
          <View style={styles.center}>
            <Text style={customTxt(Fonts.Regular, 12, colorA7A8A9).txt}>
              {age} {Translate(languageRedux).years}{' '}
              {Translate(languageRedux).OLD}
            </Text>
    ...
    return (
      <View style={styles.firstTime}>
        ...
        <Text style={[customTxt(Fonts.Regular, 16, color5C5D5E).txt, styles.ctnText]}>
          {Translate(languageRedux).FIRST_TIME_APPOINTMENT_REQUEST_DOCTOR}
        </Text>
      </View>
    )
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.ctnTitle}>
        <Text style={customTxt(Fonts.SemiBold, 16, color040404).txt}>
          {Translate(languageRedux).appoRequest}
        </Text>
        {(data || []).length > 0 && (
      ...
      {(data || []).length > 0 && renderListRequestAppointment()}
    </View>
  )
}
