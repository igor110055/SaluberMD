...
export default function DetailChronicDisease({ route }) {
  ...
  const [complication, setComplication] = useState({complication: passingData?.complication || ''})
    ...
    return (
      <>
        ...
        <CustomTextInput
          ...
          value={complication?.name || passingData?.complication}
    ...
    return (
      ...
      >
        <Text style={[
          ...
          styles.txtDelete
        ]}>{Translate(languageRedux).DELETE_DISEASE}</Text>
      </TouchableOpacity>
    )
  }
  ...
  const _onPressRight = () => {
    ...
    const callBackBD = {
      ...
      id: disease?.id,
      other: other,
      complication: complication?.name || passingData?.complication,
      complicationID: complication?.id || passingData?.complicationID,
