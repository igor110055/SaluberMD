...
import { saveLSImmunization } from '../../../actions/common'
...
import { apiPostImmunization } from 'api/MedicalRecord'
import Translate from 'translate'

export default function ImmunizationsView() {
  const immunizations = useSelector(state => state.user.immunizations)
  const [isAnswer, setAnswer] = useState((dataImmu?.datas || []).length > 0 ? true : false)
  const [isNew, setNew] = useState(false)
  const [immunization, setImmunization] = useState('')
  const [since, setSince] = useState()
  const [note, setNote] = useState('')
  const [isEmergency, setEmergency] = useState(false)
  const [tempDatas, setTemp] = useState(immunizations?.datas || [])
  ...
  const [lsImmu, setLSImmu] = useState(lsImmuRedux || [])
  ...
  useEffect(() => {
    ...
    //   setNew(true)
    // }
    setList(cloneDeep(dataImmu?.datas))
    callAPIImmu()
  }, [])

  useEffect(() => {
    const subscription = DeviceEventEmitter.addListener(Routes.IMMUNIZATION_SCREEN, params => {
  ...
  const callAPIImmu = () => {
    if (lsImmuRedux.length > 0) {return}
    setLoading(true)
    dispatch(apiHealthProfile()).then(res => {
      console.log('Res: ', res)
      const getImmu = res?.payload?.vaccini || []
      if (getImmu.length > 0) {
        setLSImmu(getImmu)
        Promise.all([
          dispatch(saveLSImmunization(getImmu))
        ])
      }
      setLoading(false)
    }).catch(() => {
  ...
  const renderQuestion = () => {
    ...
    return (
      <View style={[styles.questionView, borderBottom]}>
        ...
        ]} >{Translate(languageRedux).QUESTION_IMMU_HPW}</Text>
        <Text style={[
    ...
    return (
      <>
        <CustomTextInput
          ...
          validate={immunization?.name ? false : true}
  ...
  const _onPressAdd = () => {
    if (_.isEmpty(immunization)) {return}
    const params = {
  ...
  const checkBTAdd = () => {
    if (_.isEmpty(immunization?.name) && immunization?.id !== 1) {
      return false
    }
    if (immunization?.id === 1 && _.isEmpty(other)) {
      return false
    }
    return true
  }
  ...
  const _onPressRight = () => {
    ...
    _.forEach(tempDatas, function (value) {
      var param = {
        ...
        immunizationDate: convertDateDDMMYYYYToSever(value?.since) || '',
        remarks: value?.note || '',
        other: value?.other || '',
      ...
      setTimeout(() => {
        dispatch(apiPostImmunization(param)).then(res => {
    ...
    _.forEach(listDelete, function (value) {
      axios({
        method: 'delete',
        url: `${APIs.hostAPI}backoffice/immunization/${value?.id}`,
        headers: {
          'x-auth-token': token
        }
      })
    ...
    const renderFlatlist = () => {
      ...
      const RenderItem = ({ item, index }) => {
        const _onPressItem = () => {
          ...
          NavigationService.navigate(Routes.DETAIL_IMMUNIZATION_SCREEN, {
            data: item,
            index: index,
            lsData: list,
            lsDelete: listDelete,
    ...
    return (
      <View style={styles.lsAllerigeView}>
        <View style={[styles.questionView, borderBottom2]}>
          ...
          ]}>{Translate(languageRedux).immunization}</Text>
          <Text style={[
            customTxt(Fonts.Regular, 16, color5C5D5E).txt,
            styles.marginT8
          ]}>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do.</Text>
          ...
          </View>
        {
          ...
          >
            ...
            ]}>{Translate(languageRedux).ADD_NEW_IMMU}</Text>
          </TouchableOpacity>
          )
        }
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView style={styles.shadow}>
        {(dataImmu?.datas || []).length === 0 && isShowImmu === false && isLoading === false && renderQuestion()}
        {((dataImmu?.datas || []).length > 0 || (tempDatas || []).length > 0) && isShowImmu === false && isLoading === false && renderList()}
        {isNew && isShowImmu === false && isLoading === false && renderNewItem()}
