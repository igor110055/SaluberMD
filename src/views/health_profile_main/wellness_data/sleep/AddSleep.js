...
export default function AddSleep() {
  ...
  const checkDisable = () => {
    if ((total > 0) && (deep > 0) && (light > 0) && date && (Platform.OS === 'ios' ? hour : hourAndroid)) {
      return true
    }
    else {
      return false
  ...
  const _onPressAdd = () => {
    const hourLocal = moment((Platform.OS === 'ios' ? hour : hourAndroid)).local().format('H')
    const minutesLocal = moment((Platform.OS === 'ios' ? hour : hourAndroid)).local().format('m')
    const body = {
      value1: total,
      value2: deep,
      value3: light,
