...
import {View, Text, StyleSheet, ScrollView, DeviceEventEmitter, Platform} from 'react-native'
...
export default function AddStep() {
  ...
  const renderContent = () => {
    return (
      <View>
        <CustomTextInput
          title={Translate(languageRedux).steps}
          value={step}
          onChangeTxt={(txt) => setStep(txt)}
          textinputStyle={customTxt(Fonts.Regular, 16, color040404).txt}
          validate={step > 0 ? false : true}
    ...
    return (
      <View style={styles.marginT16}>
        <Button
          ...
          backgroundColor={step > 0 && date && (Platform.OS === 'ios' ? hour : hourAndroid) ? color3777EE : colorF0F0F0}
          textColor={step > 0 && date && (Platform.OS === 'ios' ? hour : hourAndroid) ? colorFFFFFF : colorC1C3C5}
          disabled={step > 0 && date && (Platform.OS === 'ios' ? hour : hourAndroid) ? false : true}
          onPress={_onPressAdd}
        />
      </View>
    )
  ...
  const _onPressAdd = () => {
    ...
    axios({
      method: 'post',
      url: `${APIs.hostAPI}backoffice/wellness/addWellnessValue/1`,
      headers: {
        'x-auth-token': token
      },
      data: body
