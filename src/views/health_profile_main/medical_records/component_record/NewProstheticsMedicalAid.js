...
export default function NewProstheticsMedicalAid({
  prosthetics, onPressProsthetics,
  other, setOther,
  since, onPressSince,
  note, setNote,
  ...
  const renderTextInput = () => {
    return (
      <>
        <CustomTextInput
          title={Translate(languageRedux).patientProtesi}
          value={prosthetics?.name}
          onChangeTxt={() => {}}
          placeholder={Translate(languageRedux).PH_MEDICATION}
          textStyle={styles.txtStyle}
          onPress={onPressProsthetics}
          validate={prosthetics?.id ? false : true}
          iconRight={imgHealth.ic_dropdown}
        />
        {prosthetics?.id === 1 && <CustomTextInput
  ...
  return (
    <View style={styles.container}>
      ...
      ]} >{Translate(languageRedux).NEW_PROSTHETICS}</Text>
      <Text style={[
