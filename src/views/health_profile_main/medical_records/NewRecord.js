...
import {
  StyleSheet, View, Text, DeviceEventEmitter, Platform
} from 'react-native'
...
import CustomTextInput from 'views/login_signup/components/TextInputView'
...
export default function NewRecord() {
  ...
  //Allergies
  const lsAllergi = useSelector(state => state.common.lsAllergie)
  //Medication
  const lsMedication = useSelector(state => state.common.lsMedicine)
  const [dosage, setDosage] = useState()
  const [genericName, setGenericName] = useState()
  const [medicationForm, setMedicationForm] = useState()
  const lsMedicationForm = useSelector(state => state.common.lsFormaMedicine)
  //Dependencies
  const lsType = [
  ...
  const lsHospital = useSelector(state => state.common.lsHospital)
  ...
  const lsCountry = useSelector(state => state.common.country)
  //Immunization
  const lsImmu = useSelector(state => state.common.lsImmunization)
  //Irregular
  const lsIrregular = useSelector(state => state.common.lsIrregular)
  ...
  //redux
  const AllergiesRedux = useSelector(state => state.user.allergies)
  const chronicDiseaseRedux = useSelector(state => state.user.chronic_disease)
  const medicationsRedux = useSelector(state => state.user.medications)
  const dependenciesRedux = useSelector(state => state.user.dependencies)
  const hospitalizationRedux = useSelector(state => state.user.hospitalization)
  const immunizationsRedux = useSelector(state => state.user.immunizations)
  const irregularRedux = useSelector(state => state.user.irregular)
  const prosthesisRedux = useSelector(state => state.user.prosthesis)
  const lsDiseaseRedux = useSelector(state => state.common.lsDisease)
  ...
  const renderTop = () => {
    return (
      <View style={styles.topStyle}>
        ...
        ]}>{!_.isEmpty(typeRecord)?Translate(languageRedux).category:""}</Text>
        <Text style={[
          customTxt(Fonts.Regular, 16, color5C5D5E).txt
        ]}>{_.isEmpty(typeRecord)?Translate(languageRedux).RECORD_TYPE_SELECT:""}</Text>
        <CustomTextInput
          value={typeRecord || ''}
          onChangeTxt={() => { }}
    ...
    switch (typeRecord) {
      ...
      case lsRecord[1]:
        ...
        return dispatch(apiPostAllergy(param)).then(res => {
          ...
          DeviceEventEmitter.emit('updateAllergy')
          checkResponse(res, 1)
        }).catch(() => {
    ...
    <View style={styles.container}>
      ...
      </KeyboardAwareScrollView>
      {
        isSearchList && (
          <SearchListWithName
            listData={lsRecord}
            title={`${Translate(languageRedux).SELECT_A_CATEGORY}`}
