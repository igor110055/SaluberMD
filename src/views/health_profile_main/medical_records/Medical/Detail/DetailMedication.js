...
export default function DetailMedication({ route }) {
  ...
  const lsMediRedux = useSelector(state => state.common.lsMedicine)
  const lsMediFormRedux = useSelector(state => state.common.lsFormaMedicine)
  ...
  const callAPIGetDetail = () => {
    axios({
      method: 'get',
      url: `${APIs.hostAPI}backoffice/medication/${passingData?.id}`,
      headers: {
    ...
    return (
      <View style={styles.ctnItem}>
        <CustomTextInput
          ...
          value={medication?.name || detail?.medicineName || ''}
