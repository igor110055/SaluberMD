...
export default function DetailAllergy({ route }) {
  ...
  const lsAllergiRedux = useSelector(state => state.common.lsAllergie)
  ...
  const callAPIGetDetail = () => {
    axios({
      method: 'get',
      url: `${APIs.hostAPI}backoffice/allergy/${passingData?.id}`,
      headers: {
      ...
      .then(response => {
        ...
        if (response.data.length === 0) {
          ...
          setDetail(getList)
          setAllergyID(getList?.allergyId)
        }
        setLoading(false)
      })
      .catch(error => {
    ...
    return (
      <View style={styles.ctnItem}>
        <CustomTextInput
          ...
          value={allergy?.name || detail?.allergy || ''}
