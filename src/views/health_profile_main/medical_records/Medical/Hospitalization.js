...
export default function Hospitalization({dataList}) {
  ...
  const RenderItemLong = ({item, index, data}) => {
    ...
    const checkName = () => {
      if (item.type === 0) {
        return item?.surgeryCategoryName
      } else {
        return item?.hospitalizationName
      }
    }
    const getSince = item?.hospDate ? Translate(languageRedux).date + ' ' + convertDMMMYYYY(item?.hospDate) : ''
    return (
      <View>
        <Info
          title={checkName()}
