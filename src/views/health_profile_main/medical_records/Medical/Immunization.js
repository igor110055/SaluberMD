...
export default function Immunization({dataList}) {
  ...
  const RenderItemLong = ({item, index, data}) => {
    ...
    const checkName = () => {
      if (item.immunizationId === 1) {
        return item?.other
      } else {
        return item?.immunizationName
      }
    }
    const getSince = item?.immunizationDate ? Translate(languageRedux).date + ' ' + convertDMMMYYYY(item?.immunizationDate) : ''
    return (
      <View>
        <Info
          title={checkName()}
