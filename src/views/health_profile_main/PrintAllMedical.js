...
export function PrintAllMedical() {
  ...
  const dataDisease = useSelector(state => state.user.chronic_disease)
  const dataAllergy = useSelector(state => state.user.allergies)
  const dataMedi = useSelector(state => state.user.medications)
  const dataDepen = useSelector(state => state.user.dependencies)
  const dataImmu = useSelector(state => state.user.immunizations)
  const dataIrre = useSelector(state => state.user.irregular)
  const dataPros = useSelector(state => state.user.prosthesis)
  const dataHosSur = useSelector(state => state.user.hospitalization)
  ...
  const disease = () => {
    var htmlDisease = [``]
    for (var i = 0; i <= (dataDisease || []).length - 1; i++) {
      var item =
      `<h2>${(dataDisease || []).length > i ?
      (dataDisease?.[i].diseaseId === '1' ? dataDisease?.[i].other : dataDisease?.[i].name) : ''}</h2>
      <h2>${dataDisease?.[i].startDate ? Translate(languageRedux).startDate : ''}
      ${dataDisease?.[i].startDate ? convertCalenderDDMMYYYY(dataDisease?.[i].startDate) : ''}</h2>
      <hr>
      `
      htmlDisease.push(item)
    }
    return htmlDisease
}
  ...
  const allergy = () => {
      var htmlAllergy = [``]
      for (var i = 0; i <= (dataAllergy || []).length - 1; i++) {
        var item =
        `<h2>${(dataAllergy || []).length > i ?
        (dataAllergy?.[i].allergy === 1 ? dataAllergy?.[i].other : dataAllergy?.[i].allergy) : ''}</h2>
        <h2>${dataAllergy?.[i].since ? Translate(languageRedux).since : ''}
        ${dataAllergy?.[i].since ? convertCalenderDDMMYYYY(dataAllergy?.[i].since) : ''}</h2>
        <hr>
        `
  ...
  const medication = () => {
    var htmlMedi = [``]
    for (var i = 0; i <= (dataMedi || []).length - 1; i++) {
      var item =
      `<h2>${(dataMedi || []).length > i ?
      (dataMedi?.[i].medicineId === 1 ? dataMedi?.[i].other : dataMedi?.[i].medicineName) : ''}</h2>
      <h2>${dataMedi?.[i].since ? Translate(languageRedux).since : ''}
      ${dataMedi?.[i].since ? convertCalenderDDMMYYYY(dataMedi?.[i].since) : ''}</h2>
      <hr>
      `
      htmlMedi.push(item)
    }
  ...
  const dependency = () => {
    var htmlDepen = [``]
    for (var i = 0; i <= (dataDepen || []).length - 1; i++) {
      var item =
      `<h2>${(dataDepen || []).length > i ?
      (dataDepen?.[i].dependencyId === 'Other' ? dataDepen?.[i].other : dataDepen?.[i].dependency) : ''}</h2>
      <h2>${dataDepen?.[i].yearStarted ? Translate(languageRedux).since : ''}
      ${dataDepen?.[i].yearStarted ? convertCalenderDDMMYYYY(dataDepen?.[i].yearStarted) : ''}</h2>
      <hr>
      `
      htmlDepen.push(item)
    }
  ...
  const immunization = () => {
    var htmlImmu = [``]
    for (var i = 0; i <= (dataImmu || []).length - 1; i++) {
      var item =
      `<h2>${(dataImmu || []).length > i ?
      (dataImmu?.[i].immunizationId === 'Other' ? dataImmu?.[i].other : dataImmu?.[i].immunizationName) : ''}</h2>
      <h2>${dataImmu?.[i].since ? Translate(languageRedux).date : ''}
      ${dataImmu?.[i].since ? convertCalenderDDMMYYYY(dataImmu?.[i].since) : ''}</h2>
      <hr>
      `
      htmlImmu.push(item)
    }
  ...
  const irregular = () => {
    var htmlIrre = [``]
    for (var i = 0; i <= (dataIrre || []).length - 1; i++) {
      var item =
      `<h2>${(dataIrre || []).length > i ?
      (dataIrre?.[i].testId === 1 ? dataIrre?.[i].other : dataIrre?.[i].testName) : ''}</h2>
      <h2>${dataIrre?.[i].since ? Translate(languageRedux).since : ''}
      ${dataIrre?.[i].since ? convertCalenderDDMMYYYY(dataIrre?.[i].since) : ''}</h2>
      <hr>
      `
      htmlIrre.push(item)
    }
  ...
  const prosthesis = () => {
    var htmlPros = [``]
    for (var i = 0; i <= (dataPros || []).length - 1; i++) {
      var item =
      `<h2>${(dataPros || []).length > i ?
      (dataPros[i].prosthesisId === 1 ? dataPros[i].other : dataPros[i].prosthesisName) : ''}</h2>
      <h2>${dataPros[i].since ? Translate(languageRedux).since : ''}
      ${dataPros[i].since ? convertCalenderDDMMYYYY(dataPros[i].since) : ''}</h2>
      <hr>
      `
      htmlPros.push(item)
    }
  ...
  const hossur = () => {
    var htmlhossur = [``]
    for (var i = 0; i <= (dataHosSur || []).length - 1; i++) {
      var item =
      `<h2>${(dataHosSur || []).length > i ?
      (dataHosSur?.[i].type === 1 ? dataHosSur?.[i].hospitalizationName : dataHosSur?.[i].surgerySubCategoryName) : ''}</h2>
      <h2>${dataHosSur?.[i].hospDate ? Translate(languageRedux).since : ''}
      ${dataHosSur?.[i].hospDate ? convertCalenderDDMMYYYY(dataHosSur?.[i].hospDate) : ''}</h2>
      <hr>
      `
  ...
  const html = `
        ...
        <hr>
        <h1><center>${Translate(languageRedux).CHRONIC_DISEASE}</center></h1
        <br>
        ${disease()}
