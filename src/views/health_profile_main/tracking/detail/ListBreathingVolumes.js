...
import { saveTrackingBreathingVolumes } from 'actions/common'

export default function ListBreathingVolumes({route}) {
  ...
  const callAPI = () => {
    dispatch(apiPostBreathingVolumesWellness(valShow?.value || 10, passingToken)).then(res => {
  ...
  const callAPIListManual = () => {
    axios({
      ...
      headers: {
        ...
        'x-auth-token': passingToken || token
      }
    })
      .then((response) => {
        // setLoading(false)
  ...
  return (
    <View style={styles.container}>
      ...
      {passingRoute ? <HeaderTrackingNew
        ...
        onPressRight={() => {NavigationService.navigate(Routes.SET_RANGE_SCREEN, {data: patientId, id: 9})}}
        iconRight2={icServices.ic_message}
        onPressRight2={() => {NavigationService.navigate(Routes.ALL_NOTE_SCREEN, {data: patientId, id: 9})}}
      /> : <Header
        backgroundColor={colorFFFFFF}
        textCenter={Translate(languageRedux).ARIAX}
        iconLeft={icHeader.ic_left}
      ...
      {(checkData() || []).length > 1 && valShow?.value !== 1000 && (
        <BreathingVolumeChart data={sortData()} indexNumber={valShow?.value} />
      )}
      {contentView()}
          ...
          <PlusButtonFloating
            ...
            onPressAppointment={() => {
              ...
              return NavigationService.navigate(
                Routes.ADD_BREATHING_VOLUMES_SCREEN,
              )
            }}
            onPressDirectCall={() => {
              setPlusButton(false)
