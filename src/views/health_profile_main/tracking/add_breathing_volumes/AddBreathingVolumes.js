...
import { apiPostBreathingVolumes } from 'api/DataTracking'
...
export default function AddBreathingVolumes() {
  ...
  const callAPIPost = () => {
    ...
    setLoading(true)
    dispatch(apiPostBreathingVolumes(param)).then(res => {
      console.log('apiPostBreathingVolumes1', res)

      setShowNoti(true)
      if (!(res?.payload) && res?.type === 'apiPostBreathingVolumes1') {
        setDataNoti({
        ...
        setTimeout(() => {
          DeviceEventEmitter.emit(Routes.LIST_BREATHING_VOLUMES)
        }, 4500)
        return
      }
  ...
  const renderContentInput = () => {
    return (
      <View style={styles.contentInput}>
        <CustomTextInput
          title={Translate(languageRedux).ARIAX}
          value={value}
          onChangeTxt={(txt) => { setValue(txt) }}
          placeholder={'breath / min'}
