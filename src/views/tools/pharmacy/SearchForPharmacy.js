...
const SearchListWithName = ({
  ...
  const _onPressSearch = (txt) => {
    ...
    _.forEach(cloneDB, val => {
      if (isItem) {
        ...
        if (_.includes((val?.value || '').toLowerCase(), (txt || '-').toLowerCase())) {
          data.push(val)
        }
      }
    })
  ...
  const sortData = (newDatas) => {
    ...
    const newSort = newDatas.sort(function (a, b) {
      ...
      if (isText) {
        ...
        dateA = (a.value || '').toLowerCase()
        dateB = (b.value || '').toLowerCase()
      }
      return dateB - dateA
    })
  ...
  return (
    <View style={styles.flatlistView}>
      ...
      {lsSearch.length === 0 ? <NoDataView/> : null}
        {renderHeader()}
